...
class GraphReccurent(nn.Module):
    ...
    def forward(self, x, adj):#x是节点embedding adj是A_hat
        ...
        node = hn.squeeze(0) # (1, bsz, out_features)
        return node
...
class Interact(nn.Module):
    ...
    def _construct_graph(structure):
        mids, cids, pids, time = structure#structure 是 (mids, cids, pids, time)
    ...
    def forward(self, mes_embed, structure):
        A1, A2 = self._construct_graph(structure)#structure 是 (mids, cids, pids, time)
